...
g4 <- ggplot(df_base[(df_base$v.eff == "40"),], aes(x = age, group = risk, color = risk)) +
  geom_point() + geom_line(linetype = "dotted") + labs(x = "Age group", color = "Risk group", title = "Prevalent Vaccination (40% VE)")
...
g5 <- ggplot(df_base, aes(x = dalys.averted.per100k, y = cost.diff.per100k)) +
  labs(x = "DALYs averted (per 100k)", y = "Cost (per 100k)", title =  "Cost-effectiveness Plane: Prevalent Vaccination")
...
```{r}
t <- labs(y = "$ per DALY averted", title = "Prevalent Vaccination")

...
```{r}
data <- read.csv("~/git/economic-influenza/df/icer-all.csv", as.is = TRUE)
...
df$cases.averted <- as.integer(df$cases.averted)
df$cases.averted.per100k <- as.integer(df$cases.averted.per100k)
df$icer.case.averted <- as.integer(df$icer.case.averted)

df$deaths.averted <- as.integer(df$deaths.averted)
df$deaths.averted.per100k <- as.integer(df$ deaths.averted.per100k)
df$icer.death.averted <- as.integer(df$icer.death.averted)
df$dalys <- as.integer(df$dalys)
df$dalys.per100k <- as.integer(df$dalys.per100k)
df$dalys.averted <- as.integer(df$dalys.averted)
df$dalys.averted.per100k <- as.integer(df$dalys.averted.per100k)
